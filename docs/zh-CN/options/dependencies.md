...
启用后，所有非相对导入都会**按原样标记为外部依赖，不会进行解析**。这比已废弃的 `skipNodeModulesBundle` 选项更快，即使依赖没有安装也能正常工作。请注意以下行为：

- 导入说明符会按原样保留；`my-dep/utils` 这类子路径不会被改写，`resolveDepSubpath` 选项不生效。
- 以 `#` 开头的[子路径导入](https://nodejs.org/api/packages.html#subpath-imports)仍会被解析：如果解析结果位于 `node_modules` 中，则保留原始说明符并外部化；否则打包解析到的本地文件。
- 指向本地文件的非相对导入（例如路径别名）在不解析的情况下无法识别，因此同样会被外部化。可使用 `alwaysBundle` 将它们重新打包。
